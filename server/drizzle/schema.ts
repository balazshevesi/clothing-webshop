...
export const articlePlannedSalesRelations = mysqlTable(
  ...
  (table) => {
    return {
      articlePlannedSalesRelationsId: primaryKey({
        columns: [table.id],
...
export const favItems = mysqlTable(
  "fav_items",
  {
    id: int("id").autoincrement().notNull(),
  ...
  (table) => {
    return {
      favItemsId: primaryKey({ columns: [table.id], name: "fav_items_id" }),
    };
  },
);
